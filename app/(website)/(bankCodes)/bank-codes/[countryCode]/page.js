...
//  Fetch data function
async function getBanksData(countryCode) {
  try {
...
export async function generateMetadata({ params }) {
  ...
  const canonicalUrl = new URL(`https://${host}/bank-codes/${countryCode}/`);

  try {
    const data = await getBanksData(countryCode);
          ...
          {banks.map((bank) => (
            ...
            >
              ...
              <div className="flex justify-between items-center mt-4">
                <Link
                  href={`/bank-codes/${countryCode}/${bank.bankSlug}`}
