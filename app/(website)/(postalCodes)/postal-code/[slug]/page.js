                    ...
                    <div className="space-y-1">
                        {[...Array(5)].map((_, i) => (
                            <div key={5 - i} className="flex items-center space-x-2">
                                <div className="flex">
                                    {[...Array(5)].map((_, j) => (
                                        <Icon key={j} icon="material-symbols-light:star-rounded" width="24" height="24" className={j < 5 - i ? "text-red-600 fill-red-600" : "text-gray-300"} />
                                    ))}
                                </div>
                                <div className="w-full md:w-[80%] bg-gray-300 h-4 rounded-full">
                                    {ratings[i] > 0 && (
                                        <div className="bg-red-600 h-4 rounded-full" style={{ width: `${(ratings[i] / totalReviews) * 100}%` }}></div>
                                    )}
                                </div>
        ...
        <div className="p-6 rounded-lg bg-white shadow-md">
            ...
            <div className="flex flex-wrap justify-center sm:justify-start border-b border-black mb-4 space-x-2 sm:space-x-4 pb-2">
                {["Show all", "Most Helpful", "Highest Rating", "Lowest Rating"].map((tab, index) => (
                    <button
                        key={index}
                        className={`px-3 py-2 text-sm sm:text-lg font-semibold transition rounded-2xl ${selectedReviewTab === tab ? "text-red-600 bg-gray-300" : "text-gray-500 hover:text-gray-700"
                            }`}
            ...
            <div className="border-t pt-4">
                ...
                <p className="text-xs sm:text-sm text-gray-500 text-center sm:text-left">
                    You must be{" "}
                    <span className="text-red-600 cursor-pointer hover:underline">logged in</span> to post a review.
                </p>
            </div>
        </div>
        ...
        <div>
            ...
            <div className='container mx-auto md:px-44 px-4 py-8'>
                ...
                <div className='mb-5 py-5'>
                    <h6 className='md:text-3xl text-xl font-semibold mb-3 px-2'>
                        Popular Countries:
                    </h6>
