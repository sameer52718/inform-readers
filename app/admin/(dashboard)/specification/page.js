...
const UserPage = () => {
  ...
  const getData = async (page, size) => {
    try {
      setIsLoading(true);
      const { data } = await axiosInstance.get("/admin/specification", { params: { page, limit: size } });
      if (!data.error) {
        setData(data.data);
  ...
  const handleStatusChange = async (id) => {
    try {
      const { data } = await axiosInstance.patch(`/admin/specification/${id}`);
      if (!data.error) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axiosInstance.delete(`/admin/specification/${id}`);
      if (!data.error) {
        toast.success(data.message);
        getData(pagination.currentPage, pagination.pageSize);
      } else {
