...
const UserPage = () => {
  ...
  const getData = async (page, size) => {
    try {
      setIsLoading(true);
      const { data } = await axiosInstance.get("/admin/biography", { params: { page, limit: size } });
      if (!data.error) {
        console.log(data.biographies);

        setData(data.biographies);
  ...
  const handleStatusChange = async (id) => {
    try {
      const { data } = await axiosInstance.patch(`/admin/biography/${id}`);
      if (!data.error) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axiosInstance.delete(`/admin/biography/${id}`);
      if (!data.error) {
        toast.success(data.message);
        getData(pagination.currentPage, pagination.pageSize);
      } else {
