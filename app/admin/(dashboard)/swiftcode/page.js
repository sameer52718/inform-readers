...
const UserPage = () => {
  ...
  const getData = async (page, size) => {
    try {
      setIsLoading(true);
      const { data } = await axiosInstance.get("/admin/bankCodes", { params: { page, limit: size } });
      if (!data.error) {
        setData(data.data);
  ...
  const handleStatusChange = async (id) => {
    try {
      const { data } = await axiosInstance.patch(`/admin/bankCodes/${id}`);
      if (!data.error) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axiosInstance.delete(`/admin/bankCodes/${id}`);
      if (!data.error) {
        toast.success(data.message);
        getData(pagination.currentPage, pagination.pageSize);
      } else {
  ...
  return (
    <Card>
      <div className="md:flex justify-between items-center mb-6">
        <h4 className="card-title">Bank Swift Codes</h4>
        <div className="flex gap-2">
          <GlobalFilter
            filter={globalFilter}
            setFilter={setGlobalFilter}
            placeholder="Search Bank Swift Codes..."
          />
        </div>
      </div>
      <TableBody
