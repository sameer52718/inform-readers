...
const UserPage = () => {
  ...
  const getData = async (page, size) => {
    try {
      setIsLoading(true);
      const { data } = await axiosInstance.get("/user/blog", { params: { page, limit: size } });
      if (!data.error) {
        setData(data.blogs);
  ...
  const handleStatusChange = async (id) => {
    try {
      const { data } = await axiosInstance.patch(`/user/blog/${id}`);
      if (!data.error) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axiosInstance.delete(`/user/blog/${id}`);
      if (!data.error) {
        toast.success(data.message);
        getData(pagination.currentPage, pagination.pageSize);
      } else {
    ...
    <Card>
      <div className="md:flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <GlobalFilter filter={globalFilter} setFilter={setGlobalFilter} placeholder="Search Blogs..." />
          <AddButton route={"/dashboard/blog/add"} />
        </div>
      </div>
      <TableBody
