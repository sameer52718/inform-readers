...
export default function Home() {
  ...
  const formatPeriod = (periods, frequency) => {
    const periodsPerYear = { monthly: 12, "bi-weekly": 26, weekly: 52, annually: 1 };
    const years = Math.floor(periods / periodsPerYear[frequency]);
    const remaining = periods % periodsPerYear[frequency];
    return years > 0
  ...
  const calculateLoan = () => {
    ...
    const calculation = {
      ...
      loanType,
      paymentFrequency,
      extraPayment: formatCurrency(parsedExtraPayment, currency),
      payment: formatCurrency(payment + parsedExtraPayment, currency),
      totalInterest: formatCurrency(totalInterestPaid, currency),
      payoffTime: formatPeriod(periodsTaken, paymentFrequency),
    };
  ...
  const compareScenarios = () => {
    ...
    const frequencySettings = { monthly: 12, "bi-weekly": 26, weekly: 52, annually: 1 };
    ...
    <div className="bg-white min-h-screen flex justify-center items-center p-4">
      <div className="bg-gray-100 rounded-xl shadow-2xl p-6 max-w-5xl w-full flex gap-6 flex-col md:flex-row">
        ...
        <div className="flex-1 max-h-[700px] overflow-y-auto p-5 bg-gray-200 rounded-lg">
          ...
          <div className="mt-6">
            {history.map((item, i) => (
              <div key={i} className="bg-white p-4 mb-4 rounded-lg shadow-sm">
                <p>
          ...
          {comparisonData.length > 0 && (
            <table className="w-full border-collapse bg-white rounded-lg overflow-hidden mt-6">
              <thead>
                <tr className="bg-red-500 text-white">
