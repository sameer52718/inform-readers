...
export default function Home() {
  ...
  const convertKgToOunces = () => {
    const value = parseFloat(kg);
    if (!isNaN(value)) {
      const ounces = value * 35.274;
      setResult(`Result: ${value} kg = ${ounces.toFixed(3)} ounces`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">KG to Ounces</h2>
        <input
          type="number"
          value={kg}
