...
export default function Home() {
  ...
  const convertMetersToInches = () => {
    ...
    if (!isNaN(metersValue)) {
      const inches = metersValue * 39.3701;
      setResult(`Result: ${metersValue} meters = ${inches.toFixed(3)} inches`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Meters to Inches</h2>
        <input
          type="number"
          value={meters}
