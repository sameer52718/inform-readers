...
export default function Home() {
  ...
  const convertAcresToSquareFeet = () => {
    const value = parseFloat(acres);
    if (!isNaN(value)) {
      const squareFeet = value * 43560;
      setResult(`Result: ${value} acres = ${squareFeet.toFixed(2)} square feet`);
    } else {
  ...
  return (
    <div className="bg-white  p-4 flex justify-center items-center">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl w-full max-w-xs text-center">
        <h2 className="text-2xl font-bold text-red-500 mb-5">Acres to Square Feet</h2>
        <input
          type="number"
          id="acres"
        ...
        />
        <button
          onClick={convertAcresToSquareFeet}
