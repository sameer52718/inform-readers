...
export default function Home() {
  ...
  const convertSquareFeetToSquareYards = () => {
    const value = parseFloat(squareFeet);
    if (!isNaN(value)) {
      const squareYards = value / 9;
      setResult(`Result: ${value} square feet = ${squareYards.toFixed(2)} square yards`);
    } else {
  ...
  return (
    <div className="bg-white  p-4 flex justify-center items-center">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl w-full max-w-xs text-center">
        <h2 className="text-2xl font-bold text-red-500 mb-5">Square Feet to Square Yards</h2>
        <input
          type="number"
          id="squareFeet"
        ...
        />
        <button
          onClick={convertSquareFeetToSquareYards}
