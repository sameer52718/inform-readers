...
export default function Home() {
  ...
  const handleConvert = () => {
    ...
    if (!isNaN(days)) {
      const months = days * 0.0003805175038;
      setResult(`Result: ${days} days = ${months.toFixed(6)} months`);
    } else {
  ...
  return (
    <div className="bg-white min-h-screen flex justify-center items-center">
      <div className="bg-gray-100 p-8 rounded-xl shadow-lg text-center w-full max-w-xs">
        <h2 className="text-red-500 text-2xl font-bold mb-5">Days to Months</h2>
        <input
          type="number"
          value={inputValue}
