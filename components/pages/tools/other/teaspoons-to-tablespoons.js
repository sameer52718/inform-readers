...
export default function Home() {
  ...
  const convertTeaspoonsToTablespoons = () => {
    const value = parseFloat(teaspoons);
    if (!isNaN(value)) {
      const tablespoons = value / 3;
      setResult(`Result: ${value} teaspoons = ${tablespoons.toFixed(3)} tablespoons`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Teaspoons to Tablespoons</h2>
        <input
          type="number"
          value={teaspoons}
        ...
        />
        <button
          onClick={convertTeaspoonsToTablespoons}
