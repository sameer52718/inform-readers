...
export default function Home() {
  ...
  const convertSecondsToMinutes = () => {
    ...
    if (!isNaN(value)) {
      const minutes = value * 0.0166666667;
      setResult(`Result: ${value} seconds = ${minutes.toFixed(2)} minutes`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Seconds to Minutes</h2>
        <input
          type="number"
          value={seconds}
        ...
        />
        <button
          onClick={convertSecondsToMinutes}
