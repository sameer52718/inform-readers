...
export default function Home() {
  ...
  const convertCalToKcal = () => {
    const value = parseFloat(cal);
    if (!isNaN(value)) {
      const kcal = value * 0.001;
      setResult(`Result: ${value} cal = ${kcal.toFixed(6)} kcal`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">cal to kcal</h2>
        <input
          type="number"
          value={cal}
