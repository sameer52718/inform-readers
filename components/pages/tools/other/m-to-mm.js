...
export default function Home() {
  ...
  const convertMetersToMm = () => {
    const metersValue = parseFloat(meters);
    if (!isNaN(metersValue)) {
      const mm = metersValue * 1000;
      setResult(`Result: ${metersValue} meters = ${mm.toFixed(3)} mm`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Meters to MM</h2>
        <input
          type="number"
          value={meters}
