...
export default function MpsToMphConverter() {
  const [mps, setMps] = useState("");
  const [result, setResult] = useState("Result: ");

  const convertMpsToMph = () => {
    const mpsValue = parseFloat(mps);
    if (!isNaN(mpsValue)) {
      const mph = mpsValue * 2.2369362921;
      setResult(`Result: ${mpsValue} m/s = ${mph.toFixed(2)} mph`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">m/s to mph</h2>
        <input
          type="number"
          value={mps}
