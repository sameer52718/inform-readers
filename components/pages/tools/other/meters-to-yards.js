...
export default function Home() {
  ...
  const convertMetersToYards = () => {
    ...
    if (!isNaN(metersValue)) {
      const yards = metersValue * 1.09361;
      setResult(`${metersValue} meters = ${yards.toFixed(3)} yards`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Meters to Yards</h2>
        <input
          type="number"
          value={meters}
