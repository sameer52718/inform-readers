...
export default function Home() {
  ...
  const convertCupsToTablespoons = () => {
    ...
    if (!isNaN(value)) {
      const tablespoons = value * 16;
      setResult(`Result: ${value} cups = ${tablespoons.toFixed(3)} tablespoons`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Cups to Tablespoons</h2>
        <input
          type="number"
          value={cups}
        ...
        />
        <button
          onClick={convertCupsToTablespoons}
