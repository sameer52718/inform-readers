...
export default function Home() {
  ...
  const convertMlToGallons = () => {
    const mlValue = parseFloat(ml);
    if (!isNaN(mlValue)) {
      const gallons = mlValue / 3785.41;
      setResult(`Result: ${mlValue} ml = ${gallons.toFixed(3)} gallons`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">ML to Gallon</h2>
        <input
          type="number"
          value={ml}
