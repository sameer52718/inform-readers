...
export default function Home() {
  ...
  const convertSecondsToDays = () => {
    const value = parseFloat(seconds);
    if (!isNaN(value)) {
      const days = value * 0.000011574074074;
      setResult(`Result: ${value} seconds = ${days.toFixed(6)} days`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Seconds to Days</h2>
        <input
          type="number"
          value={seconds}
