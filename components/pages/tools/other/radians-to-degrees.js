...
export default function Home() {
  ...
  const convertRadiansToDegrees = () => {
    const value = parseFloat(radians);
    if (!isNaN(value)) {
      const degrees = value * 57.2957795131;
      setResult(`Result: ${value} rad = ${degrees.toFixed(2)} °`);
    } else {
  ...
  return (
    <div className="bg-white  p-4 flex justify-center items-center">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl w-full max-w-xs text-center">
        <h2 className="text-2xl font-bold text-red-500 mb-5">Radians to Degrees</h2>
        <input
          type="number"
          id="radians"
        ...
        />
        <button
          onClick={convertRadiansToDegrees}
