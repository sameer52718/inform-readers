...
export default function Home() {
  ...
  const convertInchesToFeet = () => {
    ...
    if (!isNaN(inchesValue)) {
      const feet = inchesValue / 12;
      setResult(`Result: ${inchesValue} inches = ${feet.toFixed(3)} feet`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Inches to Feet</h2>
        <input
          type="number"
          value={inches}
