...
export default function HpToKwConverter() {
  ...
  const convertHpToKw = () => {
    ...
    if (!isNaN(hpValue)) {
      const kw = hpValue * 0.7456998715823;
      setResult(`${hpValue} hp = ${kw.toFixed(2)} kW`);
    } else {
  ...
  return (
    <div className=" bg-white flex items-center justify-center p-4">
      <div className="bg-gray-100 rounded-2xl shadow-2xl p-8 text-center w-full max-w-xs">
        <h2 className="text-2xl font-bold text-gray-800 mb-5">hp to kW</h2>
        <input
          type="number"
