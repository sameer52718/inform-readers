...
export default function Home() {
  ...
  const convertMilesToFeet = () => {
    ...
    if (!isNaN(milesValue)) {
      const feet = milesValue * 5280;
      setResult(`${milesValue} miles = ${feet.toFixed(3)} feet`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Miles to Feet</h2>
        <input
          type="number"
          value={miles}
