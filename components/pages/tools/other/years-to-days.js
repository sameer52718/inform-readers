...
export default function Home() {
  ...
  const convertYearsToDays = () => {
    const value = parseFloat(years);
    if (!isNaN(value)) {
      const days = value * 365.2425;
      setResult(`Result: ${value} years = ${days.toFixed(2)} days`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Years to Days</h2>
        <input
          type="number"
          value={years}
