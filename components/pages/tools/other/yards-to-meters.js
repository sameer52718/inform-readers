...
export default function Home() {
  ...
  const convertYardsToMeters = () => {
    ...
    if (!isNaN(yardsValue)) {
      const meters = yardsValue * 0.9144;
      setResult(`${yardsValue} yards = ${meters.toFixed(3)} meters`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Yards to Meters</h2>
        <input
          type="number"
          value={yards}
          ...
          step="any"
          className="p-2 w-[80%] mb-4 border-2 border-red-500 rounded-md text-base"
        />
        <button
          onClick={convertYardsToMeters}
