...
export default function Home() {
  ...
  const convertQuartsToCups = () => {
    ...
    if (!isNaN(quartsValue)) {
      const cups = quartsValue * 4;
      setResult(`Result: ${quartsValue} quarts = ${cups.toFixed(3)} cups`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Quart to Cups</h2>
        <input
          type="number"
          value={quarts}
