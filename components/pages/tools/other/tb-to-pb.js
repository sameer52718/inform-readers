...
export default function Home() {
  ...
  const convertTbToPb = () => {
    ...
    const pb = tbValue / 1024;
    const formattedPb = pb.toFixed(6).replace(/\.?0+$/, "");
    setResult(`Result: ${tbValue} TB = ${formattedPb} PB`);
  };

  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">TB to PB</h2>
        <input
          type="number"
          value={tb}
