...
export default function Home() {
  ...
  const convertLitersToPints = () => {
    const litersValue = parseFloat(liters);
    if (!isNaN(litersValue)) {
      const pints = litersValue / 0.473176;
      setResult(`Result: ${litersValue} liters = ${pints.toFixed(3)} pints`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Liters to Pints</h2>
        <input
          type="number"
          value={liters}
