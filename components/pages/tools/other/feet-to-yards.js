...
export default function Home() {
  ...
  const convertFeetToYards = () => {
    ...
    if (!isNaN(feetValue)) {
      const yards = feetValue / 3;
      setResult(`Result: ${feetValue} feet = ${yards.toFixed(3)} yards`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Feet to Yards</h2>
        <input
          type="number"
          value={feet}
