...
export default function Home() {
  ...
  const convertCubicFeetToCubicYards = () => {
    const value = parseFloat(cubicFeet);
    if (!isNaN(value)) {
      const cubicYards = value / 27;
      setResult(`Result: ${value} cubic feet = ${cubicYards.toFixed(3)} cubic yards`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Cubic Feet to Cubic Yards</h2>
        <input
          type="number"
          value={cubicFeet}
        ...
        />
        <button
          onClick={convertCubicFeetToCubicYards}
