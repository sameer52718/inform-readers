...
export default function Home() {
  ...
  const convertAcresToSquareMiles = () => {
    const value = parseFloat(acres);
    if (!isNaN(value)) {
      const squareMiles = value / 640;
      setResult(`Result: ${value} acres = ${squareMiles.toFixed(6)} square miles`);
    } else {
  ...
  return (
    <div className="bg-white min-h-screen p-4 flex justify-center items-center">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl w-full max-w-xs text-center">
        <h2 className="text-2xl font-bold text-red-500 mb-5">Acres to Square Miles</h2>
        <input
          type="number"
          id="acres"
        ...
        />
        <button
          onClick={convertAcresToSquareMiles}
