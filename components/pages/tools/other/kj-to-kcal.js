...
export default function KjToKcalConverter() {
  const [kj, setKj] = useState("");
  const [result, setResult] = useState("Result: ");

  const convertKjToKcal = () => {
    const kjValue = parseFloat(kj);
    if (!isNaN(kjValue)) {
      const kcal = kjValue * 0.2388459;
      setResult(`Result: ${kjValue} kJ = ${kcal.toFixed(2)} kcal`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">kJ to kcal</h2>
        <input
          type="number"
          value={kj}
