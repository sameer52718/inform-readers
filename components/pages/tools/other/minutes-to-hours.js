...
export default function Home() {
  ...
  const convertMinutesToHours = () => {
    ...
    if (!isNaN(value)) {
      const hours = value * 0.0166666667;
      setResult(`Result: ${value} minutes = ${hours.toFixed(2)} hours`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Minutes to Hours</h2>
        <input
          type="number"
          value={minutes}
