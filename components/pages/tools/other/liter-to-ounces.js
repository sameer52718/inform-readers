...
export default function LitersToOuncesConverter() {
  ...
  const convertLitersToOunces = () => {
    ...
    if (!isNaN(litersValue)) {
      const ounces = litersValue * 33.814;
      setResult(`Result: ${litersValue} liters = ${ounces.toFixed(3)} ounces`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Liters to Ounces</h2>
        <input
          type="number"
          value={liters}
