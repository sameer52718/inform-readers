...
export default function Home() {
  ...
  const convertCupsToGallon = () => {
    const value = parseFloat(cups);
    if (!isNaN(value)) {
      const gallons = value / 16;
      setResult(`Result: ${value} cups = ${gallons.toFixed(3)} gallons`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Cups to Gallons</h2>
        <input
          type="number"
          value={cups}
