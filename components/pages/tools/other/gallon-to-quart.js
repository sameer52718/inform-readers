...
export default function GallonsToQuartsConverter() {
  ...
  const convertGallonsToQuarts = () => {
    ...
    if (!isNaN(gallonsValue)) {
      const quarts = gallonsValue * 4;
      setResult(`Result: ${gallonsValue} gallons = ${quarts.toFixed(3)} quarts`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Gallons to Quarts</h2>
        <input
          type="number"
          value={gallons}
        ...
        />
        <button
          onClick={convertGallonsToQuarts}
