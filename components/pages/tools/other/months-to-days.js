...
export default function Home() {
  ...
  const convertMonthsToDays = () => {
    const value = parseFloat(months);
    if (!isNaN(value)) {
      const days = value * 30.4166666667;
      setResult(`Result: ${value} months = ${days.toFixed(2)} days`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Months to Days</h2>
        <input
          type="number"
          value={months}
