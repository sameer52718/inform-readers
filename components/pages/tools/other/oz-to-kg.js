...
export default function Home() {
  ...
  const handleConvert = () => {
    const ounces = parseFloat(inputValue);
    if (!isNaN(ounces)) {
      const kg = ounces * 0.0283495;
      setResult(`Result: ${ounces} ounces = ${kg.toFixed(3)} kg`);
    } else {
  ...
  return (
    <div className="bg-white  flex justify-center items-center">
      <div className="bg-gray-100 p-8 rounded-xl shadow-lg text-center w-full max-w-xs">
        <h2 className="text-red-500 text-2xl font-bold mb-5">Ounces to KG</h2>
        <input
          type="number"
          value={inputValue}
