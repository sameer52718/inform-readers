...
export default function LitersToCupsConverter() {
  const [liters, setLiters] = useState("");
  const [result, setResult] = useState("Result: ");

  const convertLitersToCups = () => {
    const litersValue = parseFloat(liters);
    if (!isNaN(litersValue)) {
      const cups = litersValue * 4.22675;
      setResult(`Result: ${litersValue} liters = ${cups.toFixed(3)} cups`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-lg text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Liters to Cups</h2>
        <input
          type="number"
          value={liters}
