...
export default function Home() {
  ...
  const convertKmToMeters = () => {
    const kmValue = parseFloat(km);
    if (!isNaN(kmValue)) {
      const meters = kmValue * 1000;
      setResult(`Result: ${kmValue} km = ${meters.toFixed(3)} meters`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">KM to Meters</h2>
        <input
          type="number"
          value={km}
