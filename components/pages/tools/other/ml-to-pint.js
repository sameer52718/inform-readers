...
export default function Home() {
  ...
  const convertMlToPints = () => {
    const mlValue = parseFloat(ml);
    if (!isNaN(mlValue)) {
      const pints = mlValue / 473.176;
      setResult(`Result: ${mlValue} ml = ${pints.toFixed(3)} pints`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">ML to Pint</h2>
        <input
          type="number"
          value={ml}
