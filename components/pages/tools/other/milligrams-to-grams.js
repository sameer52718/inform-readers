...
export default function Home() {
  ...
  const handleConvert = () => {
    const milligrams = parseFloat(inputValue);
    if (!isNaN(milligrams)) {
      const grams = milligrams / 1000;
      setResult(`Result: ${milligrams} milligrams = ${grams.toFixed(3)} grams`);
    } else {
  ...
  return (
    <div className="bg-white  flex justify-center items-center">
      <div className="bg-gray-100 p-8 rounded-xl shadow-lg text-center w-full max-w-xs">
        <h2 className="text-red-500 text-2xl font-bold mb-5">Milligrams to Grams</h2>
        <input
          type="number"
          value={inputValue}
