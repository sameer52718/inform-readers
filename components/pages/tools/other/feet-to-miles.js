...
export default function Home() {
  ...
  const convertFeetToMiles = () => {
    ...
    if (!isNaN(feetValue)) {
      const miles = feetValue * 0.000189394;
      setResult(`Result: ${feetValue} feet = ${miles.toFixed(6)} miles`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white">
      <div className="bg-gray-100 p-8 rounded-xl shadow-2xl text-center w-80">
        <h2 className="text-red-500 text-2xl mb-5">Feet to Miles</h2>
        <input
          type="number"
          value={feet}
