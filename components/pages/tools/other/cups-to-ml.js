...
export default function Home() {
  ...
  const convertCupsToMl = () => {
    const value = parseFloat(cups);
    if (!isNaN(value)) {
      const ml = value * 236.588;
      setResult(`Result: ${value} cups = ${ml.toFixed(3)} ml`);
    } else {
  ...
  return (
    <div className="flex justify-center items-center  bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg w-full max-w-xs text-center">
        <h2 className="text-red-500 text-2xl mb-5">Cups to ML</h2>
        <input
          type="number"
          value={cups}
